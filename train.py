...
trash_df['class'] = classes.index('trash')
# data_trash, trash_labels = prepare_jpg(trash_df)
# trash_ds = tf.data.Dataset.from_tensor_slices((data_trash, trash_labels)).batch(32)

# append to train_ds
train_ds = datagen.flow_from_dataframe(
    pd.concat([train_data, trash_df], axis=0),  
    x_col='path', 
    y_col='label',
    class_mode='categorical',
    target_size=(res_img_width, res_img_height), 
...
callbacks = [
    ...
    tf.keras.callbacks.ReduceLROnPlateau(monitor='val_categorical_accuracy', patience=4, min_lr=1e-6, factor=0.1),
